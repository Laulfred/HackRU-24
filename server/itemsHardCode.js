const items = [
    {
        UPC: 5231817601190,
        Description: "corn flour",
        Department: "Grocery",
        Price: "$1.88",
        "Store Discount": "",
        "Loyalty Discount": "$0.20",
        "Digital Coupon": "$0.30",
    },
    {
        UPC: 2979245449537,
        Description: "celery seeds",
        Department: "Grocery",
        Price: "$6.89",
        "Store Discount": "$0.25",
        "Loyalty Discount": "",
        "Digital Coupon": "$0.75",
    },
    {
        UPC: 5035131202056,
        Description: "habanero chilies",
        Department: "Produce",
        Price: "$15.06",
        "Store Discount": "$2.03",
        "Loyalty Discount": "$0.61",
        "Digital Coupon": "",
    },
    {
        UPC: 9471526586745,
        Description: "watermelons",
        Department: "Produce",
        Price: "$17.74",
        "Store Discount": "",
        "Loyalty Discount": "$2.82",
        "Digital Coupon": "",
    },
    {
        UPC: 3399182418673,
        Description: "sesame seeds",
        Department: "Grocery",
        Price: "$18.98",
        "Store Discount": "",
        "Loyalty Discount": "$4.25",
        "Digital Coupon": "",
    },
    {
        UPC: 1678817431760,
        Description: "thyme",
        Department: "Produce",
        Price: "$5.85",
        "Store Discount": "",
        "Loyalty Discount": "$0.50",
        "Digital Coupon": "$0.50",
    },
    {
        UPC: 4583127802929,
        Description: "chaurice sausage",
        Department: "Meat",
        Price: "$15.87",
        "Store Discount": "$3.82",
        "Loyalty Discount": "",
        "Digital Coupon": "$4.71",
    },
    {
        UPC: 4791906899475,
        Description: "asiago cheese",
        Department: "Deli",
        Price: "$7.05",
        "Store Discount": "$1.89",
        "Loyalty Discount": "$0.50",
        "Digital Coupon": "",
    },
    {
        UPC: 1889926696575,
        Description: "heavy cream",
        Department: "Dairy",
        Price: "$2.05",
        "Store Discount": "",
        "Loyalty Discount": "",
        "Digital Coupon": "",
    },
    {
        UPC: 4547361018113,
        Description: "gouda",
        Department: "Deli",
        Price: "$4.66",
        "Store Discount": "",
        "Loyalty Discount": "",
        "Digital Coupon": "",
    },
    {
        UPC: 8472461700373,
        Description: "ricotta cheese",
        Department: "Deli",
        Price: "$5.99",
        "Store Discount": "",
        "Loyalty Discount": "",
        "Digital Coupon": "$1.24",
    },
    {
        UPC: 2186764021114,
        Description: "gorgonzola",
        Department: "Deli",
        Price: "$18.88",
        "Store Discount": "$3.06",
        "Loyalty Discount": "$4.99",
        "Digital Coupon": "",
    },
    {
        UPC: 1482784431679,
        Description: "swiss cheese",
        Department: "Deli",
        Price: "$11.92",
        "Store Discount": "$3.69",
        "Loyalty Discount": "",
        "Digital Coupon": "$1.69",
    },
    {
        UPC: 6114996049036,
        Description: "bruschetta",
        Department: "Bakery",
        Price: "$6.24",
        "Store Discount": "",
        "Loyalty Discount": "",
        "Digital Coupon": "$3.64",
    },
    {
        UPC: 9473210774561,
        Description: "pork",
        Department: "Meat",
        Price: "$9.98",
        "Store Discount": "$4.33",
        "Loyalty Discount": "",
        "Digital Coupon": "$2.38",
    },
    {
        UPC: 1169341620944,
        Description: "sushi",
        Department: "Sushi",
        Price: "$12.99",
        "Store Discount": "",
        "Loyalty Discount": "$3.99",
        "Digital Coupon": "$1.00",
    },
    {
        UPC: 9778119292839,
        Description: "green onions",
        Department: "Produce",
        Price: "$2.32",
        "Store Discount": "",
        "Loyalty Discount": "",
        "Digital Coupon": "$0.83",
    },
    {
        UPC: 5227667456842,
        Description: "figs",
        Department: "Produce",
        Price: "$3.33",
        "Store Discount": "$0.50",
        "Loyalty Discount": "",
        "Digital Coupon": "",
    },
    {
        UPC: 1845590200208,
        Description: "leeks",
        Department: "Produce",
        Price: "$12.90",
        "Store Discount": "$0.21",
        "Loyalty Discount": "$3.14",
        "Digital Coupon": "$1.91",
    },
    {
        UPC: 1902386648243,
        Description: "sauerkraut",
        Department: "Grocery",
        Price: "$12.26",
        "Store Discount": "",
        "Loyalty Discount": "",
        "Digital Coupon": "$1.83",
    },
    {
        UPC: 3754120579944,
        Description: "avocados",
        Department: "Produce",
        Price: "$7.46",
        "Store Discount": "",
        "Loyalty Discount": "",
        "Digital Coupon": "",
    },
    {
        UPC: 4413519855575,
        Description: "garlic powder",
        Department: "Grocery",
        Price: "$4.99",
        "Store Discount": "",
        "Loyalty Discount": "$0.72",
        "Digital Coupon": "$1.24",
    },
    {
        UPC: 7711176935475,
        Description: "Tabasco sauce",
        Department: "Grocery",
        Price: "$7.76",
        "Store Discount": "$2.50",
        "Loyalty Discount": "$1.00",
        "Digital Coupon": "",
    },
    {
        UPC: 4205318194299,
        Description: "margarine",
        Department: "Dairy",
        Price: "$13.32",
        "Store Discount": "",
        "Loyalty Discount": "",
        "Digital Coupon": "$0.45",
    },
    {
        UPC: 8683025355054,
        Description: "chile peppers",
        Department: "Produce",
        Price: "$2.29",
        "Store Discount": "$0.30",
        "Loyalty Discount": "",
        "Digital Coupon": "$0.25",
    },
    {
        UPC: 7427253462404,
        Description: "maple syrup",
        Department: "Grocery",
        Price: "$14.54",
        "Store Discount": "$1.24",
        "Loyalty Discount": "",
        "Digital Coupon": "$1.13",
    },
    {
        UPC: 8138919634191,
        Description: "Parmesan cheese",
        Department: "Deli",
        Price: "$10.25",
        "Store Discount": "$2.20",
        "Loyalty Discount": "",
        "Digital Coupon": "$0.33",
    },
    {
        UPC: 3657519285082,
        Description: "pecans",
        Department: "Grocery",
        Price: "$8.07",
        "Store Discount": "",
        "Loyalty Discount": "",
        "Digital Coupon": "",
    },
    {
        UPC: 1140722978400,
        Description: "cranberries",
        Department: "Produce",
        Price: "$9.09",
        "Store Discount": "$2.63",
        "Loyalty Discount": "",
        "Digital Coupon": "$1.57",
    },
    {
        UPC: 8053119860324,
        Description: "broth",
        Department: "Grocery",
        Price: "$10.22",
        "Store Discount": "",
        "Loyalty Discount": "",
        "Digital Coupon": "",
    },
    {
        UPC: 7357282521960,
        Description: "shrimp",
        Department: "Seafood",
        Price: "$10.61",
        "Store Discount": "$4.89",
        "Loyalty Discount": "",
        "Digital Coupon": "$0.70",
    },
    {
        UPC: 2415451432799,
        Description: "squid",
        Department: "Seafood",
        Price: "$18.80",
        "Store Discount": "",
        "Loyalty Discount": "",
        "Digital Coupon": "$4.48",
    },
    {
        UPC: 8102035930705,
        Description: "horseradish",
        Department: "Grocery",
        Price: "$17.51",
        "Store Discount": "$1.23",
        "Loyalty Discount": "$1.25",
        "Digital Coupon": "",
    },
    {
        UPC: 4972205029958,
        Description: "truffles",
        Department: "Produce",
        Price: "$9.27",
        "Store Discount": "$2.33",
        "Loyalty Discount": "",
        "Digital Coupon": "",
    },
    {
        UPC: 2545417165459,
        Description: "broccoli raab",
        Department: "Produce",
        Price: "$7.62",
        "Store Discount": "$3.18",
        "Loyalty Discount": "$3.19",
        "Digital Coupon": "",
    },
    {
        UPC: 8997578872284,
        Description: "graham crackers",
        Department: "Grocery",
        Price: "$15.20",
        "Store Discount": "",
        "Loyalty Discount": "",
        "Digital Coupon": "$4.16",
    },
    {
        UPC: 5398362885863,
        Description: "cremini mushrooms",
        Department: "Produce",
        Price: "$16.68",
        "Store Discount": "$3.94",
        "Loyalty Discount": "$1.34",
        "Digital Coupon": "$2.65",
    },
    {
        UPC: 6808548120316,
        Description: "lentils",
        Department: "Produce",
        Price: "$2.59",
        "Store Discount": "$0.50",
        "Loyalty Discount": "",
        "Digital Coupon": "",
    },
    {
        UPC: 6157862106600,
        Description: "chard",
        Department: "Produce",
        Price: "$2.35",
        "Store Discount": "$0.39",
        "Loyalty Discount": "$0.25",
        "Digital Coupon": "",
    },
    {
        UPC: 1632972134587,
        Description: "tomato paste",
        Department: "Grocery",
        Price: "$14.97",
        "Store Discount": "$3.16",
        "Loyalty Discount": "",
        "Digital Coupon": "$1.57",
    },
    {
        UPC: 4111544460249,
        Description: "grouper",
        Department: "Seafood",
        Price: "$17.15",
        "Store Discount": "",
        "Loyalty Discount": "",
        "Digital Coupon": "",
    },
    {
        UPC: 8955734374901,
        Description: "cabbage",
        Department: "Produce",
        Price: "$1.76",
        "Store Discount": "$0.50",
        "Loyalty Discount": "",
        "Digital Coupon": "$0.25",
    },
    {
        UPC: 3302417478255,
        Description: "creme fraiche",
        Department: "Deli",
        Price: "$3.74",
        "Store Discount": "$1.99",
        "Loyalty Discount": "",
        "Digital Coupon": "",
    },
    {
        UPC: 2199890785599,
        Description: "liver",
        Department: "Meat",
        Price: "$12.63",
        "Store Discount": "",
        "Loyalty Discount": "",
        "Digital Coupon": "",
    },
    {
        UPC: 4234204597850,
        Description: "black olives",
        Department: "Grocery",
        Price: "$4.81",
        "Store Discount": "",
        "Loyalty Discount": "",
        "Digital Coupon": "",
    },
    {
        UPC: 4143077643737,
        Description: "beef",
        Department: "Meat",
        Price: "$13.33",
        "Store Discount": "$1.31",
        "Loyalty Discount": "",
        "Digital Coupon": "",
    },
    {
        UPC: 2575684966348,
        Description: "Havarti cheese",
        Department: "Deli",
        Price: "$7.46",
        "Store Discount": "$0.23",
        "Loyalty Discount": "",
        "Digital Coupon": "$3.48",
    },
    {
        UPC: 4561635967108,
        Description: "cashew nut",
        Department: "Grocery",
        Price: "$4.29",
        "Store Discount": "$0.99",
        "Loyalty Discount": "",
        "Digital Coupon": "$0.25",
    },
    {
        UPC: 1497996467070,
        Description: "buckwheat",
        Department: "Grocery",
        Price: "$5.76",
        "Store Discount": "",
        "Loyalty Discount": "",
        "Digital Coupon": "$1.95",
    },
    {
        UPC: 7488260790464,
        Description: "limes",
        Department: "Produce",
        Price: "$14.22",
        "Store Discount": "",
        "Loyalty Discount": "$2.95",
        "Digital Coupon": "",
    },
    {
        UPC: 5228371999833,
        Description: "snap peas",
        Department: "Produce",
        Price: "$6.96",
        "Store Discount": "",
        "Loyalty Discount": "",
        "Digital Coupon": "$4.51",
    },
    {
        UPC: 4392037881188,
        Description: "black beans",
        Department: "Grocery",
        Price: "$19.49",
        "Store Discount": "$1.96",
        "Loyalty Discount": "$4.96",
        "Digital Coupon": "",
    },
    {
        UPC: 4590380770663,
        Description: "apricots",
        Department: "Produce",
        Price: "$2.59",
        "Store Discount": "$0.25",
        "Loyalty Discount": "$0.75",
        "Digital Coupon": "$0.50",
    },
    {
        UPC: 5795082858761,
        Description: "five-spice powder",
        Department: "Grocery",
        Price: "$6.99",
        "Store Discount": "$1.00",
        "Loyalty Discount": "$2.00",
        "Digital Coupon": "",
    },
    {
        UPC: 2244194005053,
        Description: "vinegar",
        Department: "Grocery",
        Price: "$14.57",
        "Store Discount": "",
        "Loyalty Discount": "",
        "Digital Coupon": "$1.31",
    },
    {
        UPC: 7761502188786,
        Description: "cactus",
        Department: "Horticulture",
        Price: "$16.23",
        "Store Discount": "$3.24",
        "Loyalty Discount": "$2.98",
        "Digital Coupon": "$3.36",
    },
    {
        UPC: 1449919745821,
        Description: "ancho chile peppers",
        Department: "Produce",
        Price: "$16.51",
        "Store Discount": "",
        "Loyalty Discount": "",
        "Digital Coupon": "",
    },
    {
        UPC: 4449124274082,
        Description: "wasabi",
        Department: "Grocery",
        Price: "$19.62",
        "Store Discount": "",
        "Loyalty Discount": "",
        "Digital Coupon": "$4.44",
    },
    {
        UPC: 9783161777938,
        Description: "butter",
        Department: "Dairy",
        Price: "$12.28",
        "Store Discount": "$3.01",
        "Loyalty Discount": "$0.63",
        "Digital Coupon": "",
    },
    {
        UPC: 5494384947300,
        Description: "blackberries",
        Department: "Produce",
        Price: "$18.47",
        "Store Discount": "",
        "Loyalty Discount": "$3.17",
        "Digital Coupon": "$4.17",
    },
    {
        UPC: 9184752262291,
        Description: "coffee",
        Department: "Grocery",
        Price: "$14.14",
        "Store Discount": "",
        "Loyalty Discount": "$4.86",
        "Digital Coupon": "",
    },
    {
        UPC: 4324954845152,
        Description: "coriander",
        Department: "Grocery",
        Price: "$12.08",
        "Store Discount": "$2.71",
        "Loyalty Discount": "",
        "Digital Coupon": "$4.22",
    },
    {
        UPC: 1903031544014,
        Description: "brussels sprouts",
        Department: "Produce",
        Price: "$15.97",
        "Store Discount": "",
        "Loyalty Discount": "",
        "Digital Coupon": "$1.15",
    },
    {
        UPC: 7075792421200,
        Description: "sausages",
        Department: "Meat",
        Price: "$5.49",
        "Store Discount": "$3.52",
        "Loyalty Discount": "",
        "Digital Coupon": "",
    },
    {
        UPC: 2864646372952,
        Description: "brazil nuts",
        Department: "Grocery",
        Price: "$13.02",
        "Store Discount": "$4.59",
        "Loyalty Discount": "$2.73",
        "Digital Coupon": "$3.44",
    },
    {
        UPC: 2015108670094,
        Description: "trout",
        Department: "Seafood",
        Price: "$8.66",
        "Store Discount": "$3.62",
        "Loyalty Discount": "",
        "Digital Coupon": "$3.50",
    },
    {
        UPC: 2334608035251,
        Description: "cocoa powder",
        Department: "Grocery",
        Price: "$10.20",
        "Store Discount": "",
        "Loyalty Discount": "$3.55",
        "Digital Coupon": "",
    },
    {
        UPC: 4050112146065,
        Description: "mustard",
        Department: "Grocery",
        Price: "$5.81",
        "Store Discount": "",
        "Loyalty Discount": "",
        "Digital Coupon": "$2.05",
    },
    {
        UPC: 5620805255816,
        Description: "tuna",
        Department: "Grocery",
        Price: "$1.87",
        "Store Discount": "$0.25",
        "Loyalty Discount": "$0.25",
        "Digital Coupon": "$0.25",
    },
    {
        UPC: 2654586974934,
        Description: "sage",
        Department: "Produce",
        Price: "$4.99",
        "Store Discount": "",
        "Loyalty Discount": "$0.75",
        "Digital Coupon": "$0.99",
    },
    {
        UPC: 5983968397508,
        Description: "Worcestershire sauce",
        Department: "Grocery",
        Price: "$9.08",
        "Store Discount": "$1.94",
        "Loyalty Discount": "$2.92",
        "Digital Coupon": "",
    },
    {
        UPC: 7530592120376,
        Description: "dried leeks",
        Department: "Produce",
        Price: "$13.08",
        "Store Discount": "$2.08",
        "Loyalty Discount": "$1.74",
        "Digital Coupon": "$0.96",
    },
    {
        UPC: 5201423172258,
        Description: "corned beef",
        Department: "Grocery",
        Price: "$13.75",
        "Store Discount": "",
        "Loyalty Discount": "$1.71",
        "Digital Coupon": "",
    },
    {
        UPC: 9636599638490,
        Description: "red cabbage",
        Department: "Produce",
        Price: "$17.42",
        "Store Discount": "",
        "Loyalty Discount": "",
        "Digital Coupon": "",
    },
    {
        UPC: 8300166086537,
        Description: "ice cream",
        Department: "Frozen",
        Price: "$10.74",
        "Store Discount": "",
        "Loyalty Discount": "",
        "Digital Coupon": "$0.93",
    },
    {
        UPC: 6689665716108,
        Description: "sunflower seeds",
        Department: "Grocery",
        Price: "$12.70",
        "Store Discount": "$2.12",
        "Loyalty Discount": "$4.59",
        "Digital Coupon": "",
    },
    {
        UPC: 9239405530313,
        Description: "salt",
        Department: "Grocery",
        Price: "$4.49",
        "Store Discount": "",
        "Loyalty Discount": "",
        "Digital Coupon": "$0.36",
    },
    {
        UPC: 8984654669346,
        Description: "steak",
        Department: "Meat",
        Price: "$16.08",
        "Store Discount": "",
        "Loyalty Discount": "",
        "Digital Coupon": "",
    },
    {
        UPC: 6465537833829,
        Description: "spaghetti squash",
        Department: "Produce",
        Price: "$13.09",
        "Store Discount": "$4.30",
        "Loyalty Discount": "$3.12",
        "Digital Coupon": "",
    },
    {
        UPC: 4534898125201,
        Description: "potato chips",
        Department: "Grocery",
        Price: "$9.91",
        "Store Discount": "$0.84",
        "Loyalty Discount": "",
        "Digital Coupon": "$0.53",
    },
    {
        UPC: 1442350419894,
        Description: "onions",
        Department: "Produce",
        Price: "$11.16",
        "Store Discount": "",
        "Loyalty Discount": "",
        "Digital Coupon": "$2.70",
    },
    {
        UPC: 5063051286197,
        Description: "lobsters",
        Department: "Seafood",
        Price: "$17.47",
        "Store Discount": "$2.01",
        "Loyalty Discount": "",
        "Digital Coupon": "$1.90",
    },
    {
        UPC: 7953214686934,
        Description: "hot sauce",
        Department: "Grocery",
        Price: "$16.28",
        "Store Discount": "",
        "Loyalty Discount": "",
        "Digital Coupon": "$3.41",
    },
    {
        UPC: 8039368620360,
        Description: "blueberries",
        Department: "Produce",
        Price: "$6.80",
        "Store Discount": "",
        "Loyalty Discount": "",
        "Digital Coupon": "$3.93",
    },
    {
        UPC: 3375611948338,
        Description: "vanilla bean",
        Department: "Grocery",
        Price: "$9.92",
        "Store Discount": "$4.84",
        "Loyalty Discount": "$3.68",
        "Digital Coupon": "",
    },
    {
        UPC: 5993137352730,
        Description: "cantaloupes",
        Department: "Produce",
        Price: "$6.41",
        "Store Discount": "$2.90",
        "Loyalty Discount": "$2.22",
        "Digital Coupon": "",
    },
    {
        UPC: 8150313127959,
        Description: "capers",
        Department: "Produce",
        Price: "$7.32",
        "Store Discount": "",
        "Loyalty Discount": "$1.24",
        "Digital Coupon": "",
    },
    {
        UPC: 3140439829416,
        Description: "sweet chili sauce",
        Department: "Grocery",
        Price: "$3.98",
        "Store Discount": "",
        "Loyalty Discount": "",
        "Digital Coupon": "$1.00",
    },
    {
        UPC: 5290029791815,
        Description: "water",
        Department: "Grocery",
        Price: "$3.21",
        "Store Discount": "$0.50",
        "Loyalty Discount": "$0.75",
        "Digital Coupon": "$0.50",
    },
    {
        UPC: 1235751160277,
        Description: "bean sauce",
        Department: "Grocery",
        Price: "$13.70",
        "Store Discount": "",
        "Loyalty Discount": "$3.61",
        "Digital Coupon": "",
    },
    {
        UPC: 7547001678932,
        Description: "portabella mushrooms",
        Department: "Produce",
        Price: "$7.17",
        "Store Discount": "$3.60",
        "Loyalty Discount": "$2.31",
        "Digital Coupon": "",
    },
];

const determineSustainability = (upc) => {
    const itemT = items.find((i) => upc === i.UPC);
    // console.log(itemT.Description);
    isSustainable = matchItem(itemT);
    return ({isSustainable: isSustainable.split(" ")[0] === "Sustainable" ? true : false, message: isSustainable, foodItem: itemT});
};

module.exports.getSustainability = function (upc) {
    return determineSustainability(upc);
};

module.exports.items = items;


function matchItem(itemT) {
    switch (itemT.Description) {
        case "corn flour":
            return "Sustainable because it is a plant-based product";
        case "celery seeds":
            return "Sustainable because it is a plant-based product";
        case "habanero chilies":
            return "Sustainable because it is a plant-based product";
        case "watermelons":
            return "Sustainable because it is a plant-based product";
        case "sesame seeds":
            return "Sustainable because it is a plant-based product";
        case "thyme":
            return "Sustainable because it is a plant-based product";
        case "chaurice sausage":
            return "Not sustainable because it is a meat product";
        case "asiago cheese":
            return "Not sustainable because it is a dairy product";
        case "heavy cream":
            return "Not sustainable because it is a dairy product";
        case "gouda":
            return "Not sustainable because it is a dairy product";
        case "ricotta cheese":
            return "Not sustainable because it is a dairy product";
        case "gorgonzola":
            return "Not sustainable because it is a dairy product";
        case "swiss cheese":
            return "Not sustainable because it is a dairy product";
        case "bruschetta":
            return "Not sustainable because it is a bakery product";
        case "pork":
            return "Not sustainable because it is a meat product";
        case "sushi":
            return "Not sustainable because it is a seafood product";
        case "green onions":
            return "Sustainable because it is a plant-based product";
        case "figs":
            return "Sustainable because it is a plant-based product";
        case "leeks":
            return "Sustainable because it is a plant-based product";
        case "sauerkraut":
            return "Sustainable because it is a plant-based product";
        case "avocados":
            return "Sustainable because it is a plant-based product";
        case "garlic powder":
            return "Sustainable because it is a plant-based product";
        case "Tabasco sauce":
            return "Sustainable because it is a plant-based product";
        case "margarine":
            return "Not sustainable because it is a dairy product";
        case "chile peppers":
            return "Sustainable because it is a plant-based product";
        case "maple syrup":
            return "Sustainable because it is a plant-based product";
        case "Parmesan cheese":
            return "Not sustainable because it is a dairy product";
        case "pecans":
            return "Sustainable because it is a plant-based product";
        case "cranberries":
            return "Sustainable because it is a plant-based product";
        case "broth":
            return "Not sustainable because it is a animal-based product";
        case "shrimp":
            return "Not sustainable because it is a seafood product";
        case "squid":
            return "Not sustainable because it is a seafood product";
        case "horseradish":
            return "Sustainable because it is a plant-based product";
        case "truffles":
            return "Sustainable because it is a plant-based product";
        case "broccoli raab":
            return "Sustainable because it is a plant-based product";
        case "graham crackers":
            return "Not sustainable because it is a bakery product";
        case "cremini mushrooms":
            return "Sustainable because it is a plant-based product";
        case "lentils":
            return "Sustainable because it is a plant-based product";
        case "chard":
            return "Sustainable because it is a plant-based product";
        case "tomato paste":
            return "Sustainable because it is a plant-based product";
        case "grouper":
            return "Not sustainable because it is a seafood product";
        case "cabbage":
            return "Sustainable because it is a plant-based product";
        case "creme fraiche":
            return "Not sustainable because it is a dairy product";
        case "liver":
            return "Not sustainable because it is a meat product";
        case "black olives":
            return "Sustainable because it is a plant-based product";
        case "beef":
            return "Not sustainable because it is a meat product";
        case "Havarti cheese":
            return "Not sustainable because it is a dairy product";
        case "cashew nut":
            return "Sustainable because it is a plant-based product";
        case "buckwheat":
            return "Sustainable because it is a plant-based product";
        case "limes":
            return "Sustainable because it is a plant-based product";
        case "snap peas":
            return "Sustainable because it is a plant-based product";
        case "black beans":
            return "Sustainable because it is a plant-based product";
        case "apricots":
            return "Sustainable because it is a plant-based product";
        case "five-spice powder":
            return "Sustainable because it is a plant-based product";
        case "vinegar":
            return "Sustainable because it is a plant-based product";
        case "cactus":
            return "Sustainable because it is a plant-based product";
        case "ancho chile peppers":
            return "Sustainable because it is a plant-based product";
        case "wasabi":
            return "Sustainable because it is a plant-based product";
        case "butter":
            return "Not sustainable because it is a dairy product";
        case "blackberries":
            return "Sustainable because it is a plant-based product";
        case "coffee":
            return "Sustainable because it is a plant-based product";
        case "coriander":
            return "Sustainable because it is a plant-based product";
        case "brussels sprouts":
            return "Sustainable because it is a plant-based product";
        case "sausages":
            return "Not sustainable because it is a meat product";
        case "brazil nuts":
            return "Sustainable because it is a plant-based product";
        case "trout":
            return "Not sustainable because it is a seafood product";
        case "cocoa powder":
            return "Sustainable because it is a plant-based product";
        case "mustard":
            return "Sustainable because it is a plant-based product";
        case "tuna":
            return "Not sustainable because it is a seafood product";
        case "sage":
            return "Sustainable because it is a plant-based product";
        case "Worcestershire sauce":
            return "Sustainable because it is a plant-based product";
        case "dried leeks":
            return "Sustainable because it is a plant-based product";
        case "corned beef":
            return "Not sustainable because it is a meat product";
        case "red cabbage":
            return "Sustainable because it is a plant-based product";
        case "ice cream":
            return "Not sustainable because it is a dairy product";
        case "sunflower seeds":
            return "Sustainable because it is a plant-based product";
        case "salt":
            return "Sustainable because it is a mineral-based product";
        case "steak":
            return "Not sustainable because it is a meat product";
        case "spaghetti squash":
            return "Sustainable because it is a plant-based product";
        case "potato chips":
            return "Not sustainable because it is a plant-based product";
        case "onions":
            return "Sustainable because it is a plant-based product";
        case "lobsters":
            return "Not sustainable because it is a seafood product";
        case "hot sauce":
            return "Sustainable because it is a plant-based product";
        case "blueberries":
            return "Sustainable because it is a plant-based product";
        case "vanilla bean":
            return "Sustainable because it is a plant-based product";
        case "cantaloupes":
            return "Sustainable because it is a plant-based product";
        case "capers":
            return "Sustainable because it is a plant-based product";
        case "sweet chili sauce":
            return "Sustainable because it is a plant-based product";
        case "water":
            return "Sustainable";
        case "bean sauce":
            return "Sustainable because it is a plant-based product";
        case "portabella mushrooms":
            return "Sustainable because it is a plant-based product";
        default:
            return "Not enough information to determine sustainability";
    }
}
/*
const getInput = (item) => {
  switch (item.Description) {
    case "corn flour":
      input = "is corn flour sustainable?";
    case "celery seeds":
      input = "are celery seeds sustainable?";
    case "habanero chilies":
      input = "are habanero chilies sustainable?";
    case "watermelons":
      input = "are watermelons sustainable?";
    case "sesame seeds":
      input = "are sesame seeds sustainable?";
    case "thyme":
      input = "is thyme sustainable?";
    case "chaurice sausage":
      input = "is chaurice sausage sustainable?";
    case "asiago cheese":
      input = "is asiago cheese sustainable?";
    case "heavy cream":
      input = "is heavy cream sustainable?";
    case "gouda":
      input = "is gouda sustainable?";
    case "ricotta cheese":
      input = "is ricotta cheese sustainable?";
    case "gorgonzola":
      input = "is gorgonzola sustainable?";
    case "swiss cheese":
      input = "is swiss cheese sustainable?";
    case "bruschetta":
      input = "is bruschetta sustainable?";
    case "pork":
      input = "is pork sustainable?";
    case "sushi":
      input = "is sushi sustainable?";
    case "green onions":
      input = "are green onions sustainable?";
    case "figs":
      input = "are figs sustainable?";
    case "leeks":
      input = "are leeks sustainable?";
    case "sauerkraut":
      input = "is sauerkraut sustainable?";
    case "avocados":
      input = "are avocados sustainable?";
    case "garlic powder":
      input = "is garlic powder sustainable?";
    case "Tabasco sauce":
      input = "is Tabasco sauce sustainable?";
    case "margarine":
      input = "is margarine sustainable?";
    case "chile peppers":
      input = "are chile peppers sustainable?";
    case "maple syrup":
      input = "is maple syrup sustainable?";
    case "Parmesan cheese":
      input = "is Parmesan cheese sustainable?";
    case "pecans":
      input = "are pecans sustainable?";
    case "cranberries":
      input = "are cranberries sustainable?";
    case "broth":
      input = "is broth sustainable?";
    case "shrimp":
      input = "are shrimp sustainable?";
    case "squid":
      input = "is squid sustainable?";
    case "horseradish":
      input = "is horseradish sustainable?";
    case "truffles":
      input = "are truffles sustainable?";
    case "broccoli raab":
      input = "is broccoli raab sustainable?";
    case "graham crackers":
      input = "are graham crackers sustainable?";
    case "cremini mushrooms":
      input = "are cremini mushrooms sustainable?";
    case "lentils":
      input = "are lentils sustainable?";
    case "chard":
      input = "is chard sustainable?";
    case "tomato paste":
      input = "is tomato paste sustainable?";
    case "grouper":
      input = "is grouper sustainable?";
    case "cabbage":
      input = "is cabbage sustainable?";
    case "creme fraiche":
      input = "is creme fraiche sustainable?";
    case "liver":
      input = "is liver sustainable?";
    case "black olives":
      input = "are black olives sustainable?";
    case "beef":
      input = "is beef sustainable?";
    case "Havarti cheese":
      input = "is Havarti cheese sustainable?";
    case "cashew nut":
      input = "are cashew nuts sustainable?";
    case "buckwheat":
      input = "is buckwheat sustainable?";
    case "limes":
      input = "are limes sustainable?";
    case "snap peas":
      input = "are snap peas sustainable?";
    case "black beans":
      input = "are black beans sustainable?";
    case "apricots":
      input = "are apricots sustainable?";
    case "five-spice powder":
      input = "is five-spice powder sustainable?";
    case "vinegar":
      input = "is vinegar sustainable?";
    case "cactus":
      input = "is cactus sustainable?";
    case "ancho chile peppers":
      input = "are ancho chile peppers sustainable?";
    case "wasabi":
      input = "is wasabi sustainable?";
    case "butter":
      input = "is butter sustainable?";
    case "blackberries":
      input = "are blackberries sustainable?";
    case "coffee":
      input = "is coffee sustainable?";
    case "coriander":
      input = "is coriander sustainable?";
    case "brussels sprouts":
      input = "are brussels sprouts sustainable?";
    case "sausages":
      input = "are sausages sustainable?";
    case "brazil nuts":
      input = "are brazil nuts sustainable?";
    case "trout":
      input = "is trout sustainable?";
    case "cocoa powder":
      input = "is cocoa powder sustainable?";
    case "mustard":
      input = "is mustard sustainable?";
    case "tuna":
      input = "is tuna sustainable?";
    case "sage":
      input = "is sage sustainable?";
    case "Worcestershire sauce":
      input = "is Worcestershire sauce sustainable?";
    case "dried leeks":
      input = "are dried leeks sustainable?";
    case "corned beef":
      input = "is corned beef sustainable?";
    case "red cabbage":
      input = "is red cabbage sustainable?";
    case "ice cream":
      input = "is ice cream sustainable?";
    case "sunflower seeds":
      input = "are sunflower seeds sustainable?";
    case "salt":
      input = "is salt sustainable?";
    case "steak":
      input = "is steak sustainable?";
    case "spaghetti squash":
      input = "is spaghetti squash sustainable?";
    case "potato chips":
      input = "are potato chips sustainable?";
    case "onions":
      input = "are onions sustainable?";
    case "lobsters":
      input = "are lobsters sustainable?";
    case "hot sauce":
      input = "is hot sauce sustainable?";
    case "blueberries":
      input = "are blueberries sustainable?";
    case "vanilla bean":
      input = "is vanilla bean sustainable?";
    case "cantaloupes":
      input = "are cantaloupes sustainable?";
    case "capers":
      input = "are capers sustainable?";
    case "sweet chili sauce":
      input = "is sweet chili sauce sustainable?";
    case "water":
      input = "is water sustainable?";
    case "bean sauce":
      input = "is bean sauce sustainable?";
    case "portabella mushrooms":
      input = "are portabella mushrooms sustainable?";
    default:
      input = "Tell me more about sustainability";
  }
}



async function run(model, input) {
  const response = await fetch(
    `https://api.cloudflare.com/client/v4/accounts/6cbc3e4ecf126052588f1e2d0de14076/ai/run/@cf/meta/llama-2-7b-chat-int8`,
    {
      headers: { Authorization: "Bearer 8d90d9c6541816abf6257cab0f41711c9ed77" },
      method: "POST",
      body: JSON.stringify(input),
    }
  );
  const result = await response.json();
  return result;
}


const main = async () => {
  const model = "llama-2-7b-chat-int8";
  const input = "is corn flour sustainable?";
  const result = await run(model, input);
  console.log(result);
}

main();
*/
